...
#' \code{identity::identity.coefs}, which is an R interface for the C
...
#' provided an alternative wrapper, \code{jacquard2}, which executes an 
#' external call to the original C program \code{IdCoefs} (version 2.1.1). 
#' For this to function, \code{IdCoefs} must be installed on the computer 
#' (see link in the References section below) and the executable placed in a 
#' folder included in the PATH variable. The \code{jacquard2} wrapper works 
#' by writing the necessary files to disk and calling \code{IdCoefs} via 
#' \code{\link{system}}.
#' 
...
#' @seealso \code{\link[kinship2]{kinship}}, \code{\link[identity]{identity.coefs}}
